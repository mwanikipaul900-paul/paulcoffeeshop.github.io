...
async function getMpesaAccessToken() {
    ...
    if (!key || !secret) throw new Error('Missing M-Pesa consumer key or secret');
...
async function initiateStkPush({ amount, phoneNumber, accountReference }) {
    ...
    if (!shortcode || !passkey || !callbackUrl) throw new Error('Missing M-Pesa shortcode, passkey, or callback URL');
    ...
    const body = {
        ...
        TransactionDesc: 'Paul Coffee Shop payment'
    };
    ...
    const data = await res.json();
    if (!res.ok) throw new Error(data.errorMessage || data.errorMessage || 'STK push failed');
    return data;
}
...
app.post('/api/create-payment-link', async (req, res) => {
    try {
        ...
        const tapCurrency = paymentMethod === 'knet' ? 'KWD' : 'KWD';
        ...
        const tapPayload = {
            ...
            threeDSecure: true,
            save_card: false,
            description: `Paul Coffee Shop Order ${orderId}`,
            statement_descriptor: 'PAUL COFFEE',
            customer: {
            ...
            redirect: {
                url: `${process.env.BASE_URL || `http://localhost:${PORT}`}/payment-success?orderId=${encodeURIComponent(orderId)}`
            },
            post: {
                url: `${process.env.BASE_URL || `http://localhost:${PORT}`}/api/tap-webhook`
            },
            reference: {
                transaction: orderId,
                order: orderId
        ...
        const paymentUrl =
            tapData?.transaction?.url ||
            tapData?.url ||
            tapData?.redirect?.url ||
            null;
